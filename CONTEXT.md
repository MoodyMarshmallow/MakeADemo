...
- **Repo Security Screen**: The read-only Pipeline stage that performs a fast deterministic static scan and then requires one tool-free agent verdict over bounded static evidence before any runtime preparation work begins.
...
- **Repo Security Screen** runs before **Repo Preparation**. Its deterministic scan rejects only an exact known destructive command; otherwise a separate tool-free agent review is the sole approval transition into Repo Preparation.
...
- The Repo Security review receives only backend-selected bounded static evidence, has no tools or Sandbox/browser access, and cannot pass warnings or repository text into Repo Preparation as authority.
