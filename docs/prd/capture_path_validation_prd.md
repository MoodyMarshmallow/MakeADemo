...
MakeADemo needs a single non-agent validation gate that proves the prepared app and generated Demo Script work together before Footage Capture begins, while still preserving clear stage contracts, backend-controlled trust boundaries, and fast validation.
...
Capture Path Validation is a dry run. It skips presentation-oriented behavior such as human-like typing, visible cursor movement, and recording-specific pauses so validation stays fast and deterministic. Footage Capture runs after Capture Path Validation succeeds and starts from fresh deterministic app state, using the accepted Demo Script to record final Scenes with the slower presentation-oriented browser behavior.

If Capture Path Validation fails, the backend returns structured failure feedback to the same agent and allows repair attempts. The repair budget defaults to a small bounded value and is configurable through `MAKEADEMO_CAPTURE_PATH_REPAIR_ATTEMPTS`. Every repair retry must rerun the full Capture Path Validation stage from the beginning, including Demo Runtime Preflight and capture-path checks. If the repair budget is exhausted, the Pipeline Job fails and tells the user to report the issue to MakeADemo rather than returning a partially trusted Demo Script or a Preparation Fallback Prompt.
...
12. As a maker, I want MakeADemo to avoid returning a Demo Script that has not passed Capture Path Validation, so that downstream capture does not treat untrusted scripts as ready.
...
35. As a MakeADemo operator, I want the accepted Demo Script to be stored only after Capture Path Validation succeeds, so that persisted script artifacts represent capture-ready output.
...
- Script Generation produces a Demo Script.
- Capture Path Validation should produce validation evidence and an accepted or failed result.
- The backend validator remains authoritative over whether a Demo Script is accepted for Footage Capture.
- If Capture Path Validation fails, the backend may send structured failure feedback to the same Agent Session.
- The agent may repair the prepared workspace, the Demo Script, or both after Capture Path Validation failure.
...
- Exhausted repair attempts should not return a partially trusted Demo Script.
...
- The same accepted Demo Script should drive Capture Path Validation and Footage Capture.
...
- Existing Demo Runtime Preflight adapters and browser validation behavior are reused inside Capture Path Validation where they remain valuable.
...
- The Demo Script is the handoff artifact from Script Generation into Capture Path Validation, Footage Capture, and Compositing.
...
- Tests should prove that the agent may repair the Demo Script and rerun Capture Path Validation.
...
- Tests should prove that exhausted repair attempts do not return or persist a capture-ready Demo Script.
...
- Existing Demo Runtime Preflight and Browser Validation tests should be reused or migrated so their behavior is preserved inside Capture Path Validation.
