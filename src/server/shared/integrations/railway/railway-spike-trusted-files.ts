/**
 * Trusted template files rendered by the Railway gateway before recipe
 * commands run. They must remain root-owned and never be derived from a
 * submitted workspace.
 */
export const railwaySpikeTrustedFiles = [
  {
    contents: String.raw`#!/usr/bin/env node
...
const acceptedFiles = new Set([
  ...
  "pnpm-lock.yaml", "yarn.lock", "bun.lock", "bun.lockb", ".nvmrc",
  ".node-version", ".tool-versions", "mise.toml", ".mise.toml",
]);
...
const rootPackage = await boundedLstatIfExists(posix.join(workspace, "package.json"));
const candidates = rootPackage === undefined
...
async function discoverNestedCandidates() {
  ...
  for (const projectRoot of roots.sort()) {
    ...
    if (candidates.length > maxCandidates) throw new Error("too many JavaScript project roots");
  }
  return candidates;
}
...
async function readRootFiles(packageDetails) {
  ...
  for (const name of acceptedFiles) {
    const details = name === "package.json"
      ? packageDetails
      : await boundedLstatIfExists(posix.join(workspace, name));
    if (details !== undefined) await readMetadataFile(files, workspace, ".", name, details);
  }
  return files;
}
...
async function readNamedFiles(directory, projectRoot, names) {
  ...
  for (const name of names) {
    ...
    await readMetadataFile(files, directory, projectRoot, name, await boundedLstat(posix.join(directory, name)));
  }
  return files;
}
...
async function readMetadataFile(files, directory, projectRoot, name, details) {
  ...
  if (presenceOnlyFiles.has(name)) {
    files[name] = await readCanonicalLockEvidence(posix.join(directory, name), projectRoot, name);
    return;
  }
  if (details.size > maxFileBytes) throw new Error("unsafe toolchain metadata file: " + projectRoot + "/" + name);
...
async function readCanonicalLockEvidence(path, projectRoot, name) {
  ...
  try {
    ...
    const prefix = Buffer.allocUnsafe(Math.min(initial.size, maxLockfilePrefixBytes));
    const chunk = Buffer.allocUnsafe(64 * 1024);
    let position = 0;
    let prefixBytes = 0;
    while (position < initial.size) {
      const { bytesRead } = await handle.read(chunk, 0, Math.min(chunk.length, initial.size - position), position);
      if (bytesRead === 0) throw new Error("unsafe toolchain metadata file: " + projectRoot + "/" + name);
...
async function safeDirectories(directory) {
  ...
  for (const name of await safeNames(directory)) {
    if (!/^[A-Za-z0-9._-]+$/.test(name) || name === "node_modules" || name === ".git") continue;
...
async function boundedLstatIfExists(path) {
  try { return await boundedLstat(path); }
  catch (error) {
...
function assertTraversalBudget() {
  if (visitedDirectoryEntries > maxDirectoryEntries || visitedRoots > maxVisitedRoots || performedLstats > maxLstats) {
